...
MAX_CHANNUM_V40 = 512  # 最大支持的通道数（V40版本）
...
MAX_IP_DEVICE_V40 = 64  # 最大IP设备数（V40版本）
...
class NET_DVR_IPADDR(Structure):
    _fields_ = [
        ('sIpV4', c_char * 16),  # IPv4地址
        ('byIPv6', c_byte * 128),  # 保留
    ]
...
class NET_DVR_VOD_PARA(Structure):
    _fields_ = [
        ...
        ("hWnd", c_uint32),
        ("byDrawFrame", c_uint64),  # 0:不抽帧，1：抽帧
        ("byVolumeType", c_uint32),  # 0-普通录像卷  1-存档卷
        ("byVolumeNum", c_ubyte),  # 卷号，目前指存档卷号
        ("byStreamType", c_ubyte),  # 码流类型 0-主码流， 1-子码流，2-码流三
        ("dwFileIndex", c_ubyte),  # 存档卷上的录像文件索引，搜索存档卷录像时返回的值
        ("byAudioFile", c_ubyte),  # 音频文件0-否，1-是
        ("byCourseFile", c_ubyte),  # 课程文件0-否，1-是
        ("byDownload", c_ubyte),  # 是否下载 0-否，1-是
        ("byOptimalStreamType", c_ubyte),  # 是否按最优码流类型回放 0-否，1-是（对于双码流设备，某一段时间内的录像文件与指定码流类型不同，则返回实际码流类型的录像）
        ("byRes2", c_ubyte * 20),  # 预留
    ]
...
class NET_DVR_TIME_SEARCH_COND(Structure):
    _fields_ = [
        ("wYear", c_uint16),  # 年，设备OSD时间
        ("byMonth", c_ubyte),  # 月，设备OSD时间
        ("byDay", c_ubyte),  # 日，设备OSD时间
        ("byHour", c_ubyte),  # 时，设备OSD时间
        ("byMinute", c_ubyte),  # 分，设备OSD时间
        ("bySecond", c_ubyte),  # 秒，设备OSD时间
        ("byLocalOrUTC", c_ubyte),  # 0-设备本地时间，即设备OSD时间；1-UTC时间
        ("wMillisecond", c_uint16),  # 毫秒，精度不够，默认为0
        ("cTimeDifferenceH", c_char),  # 与UTC的时差（小时），-12 ... +14，+表示东区，byLocalOrUTC为1时有效
        ("cTimeDifferenceM", c_char),  # 与UTC的时差（分钟），-30, 0, 30, 45，+表示东区，byLocalOrUTC为1时有效
    ]
...
class NET_DVR_TIME_SEARCH(Structure):
    _fields_ = [
        ...
        ("byMinute", c_ubyte),  # 分，设备OSD时间
        ("bySecond", c_ubyte),  # 秒，设备OSD时间
        ("cTimeDifferenceH", c_char),  # 与国际标准时间的时差（小时），-12 ... +14
        ("cTimeDifferenceM", c_char),  # 与国际标准时间的时差（分钟），-30, 0, 30, 45
        ("byRes", c_ubyte * 3),  # 保留，置为0
    ]
...
class NET_DVR_ATMFINDINFO(Structure):
    _fields_ = [
        ("byTransactionType", c_ubyte),
        # 交易类型：0- 全部，1- 查询，2- 取款，3- 存款，4- 修改密码，5- 转账，6- 无卡查询，7- 无卡存款，8- 吞钞，9- 吞卡，10- 自定义
        ("byRes", c_ubyte * 3),  # 保留，置为0
        ("dwTransationAmount", c_uint32),  # 交易金额，-1表示查找时不带金额
    ]
...
class NET_DVR_FILECOND_V50(Structure):
    _fields_ = [
        ...
        ("byFindType", c_ubyte),
        ("byDrawFrame", c_ubyte),
        ("byQuickSearch", c_ubyte),  # 0:不抽帧，1：抽帧
        ("byStreamType", c_ubyte),  # 0-普通录像卷  1-存档卷
        ("dwFileType", c_uint32),  # 卷号，目前指存档卷号
        ("dwVolumeNum", c_uint32),  # 存档卷上的录像文件索引，搜索存档卷录像时返回的值
        ("byIsLocked", c_ubyte),  # 音频文件0-否，1-是
        ("byNeedCard", c_ubyte),  # 课程文件0-否，1-是
        ("byOnlyAudioFile", c_ubyte),  # 是否下载 0-否，1-是
        ("bySpecialFindInfoType", c_ubyte),  # 是否按最优码流类型回放 0-否，1-是（对于双码流设备，某一段时间内的录像文件与指定码流类型不同，则返回实际码流类型的录像）
        ("szCardNum", c_char * 32),  # 预留
        ("szWorkingDeviceGUID", c_char * 16),  # 预留
        ("uSpecialFindInfo", NET_DVR_SPECIAL_FINDINFO_UNION),  # 预留
        ("dwTimeout", c_uint32),  # 预留
        ("byRes", c_ubyte * 20),  # 预留
    ]
...
class NET_DVR_FINDDATA_V50(Structure):
    _fields_ = [
        ...
        ("dwFileSize", c_uint32),  # 文件大小 //对于大文件搜索类型时（byBigFileType为1），该值无意义
        ("byLocked", c_ubyte),  # 文件是否被锁定，1－文件已锁定；0－文件未锁定
        ("byFileType", c_ubyte),  # 文件类型，与V40相同
        ("byQuickSearch", c_ubyte),  # 0- 普通查询结果，1- 快速（日历）查询结果
        ("byStreamType", c_ubyte),  # 码流类型：0- 主码流，1- 子码流，2- 码流三
        ("dwFileIndex", c_uint32),  # 文件索引号
        ("sCardNum", c_char * 32),  # 卡号
        ("dwTotalLenH", c_uint32),  # 对于大文件搜索，时间段内数据总长度，高32字节
        ("dwTotalLenL", c_uint32),  # 对于大文件搜索，时间段内数据总长度，低32字节
        ("byBigFileType", c_ubyte),  # 0为普通片段搜索，1为大文件搜索
        ("byRes", c_ubyte * 247),  # 预留
    ]
...
class NET_DVR_IPDEVINFO_V31(Structure):
    _fields_ = [
        ("byEnable", c_ubyte),  # 该IP设备是否有效
        ("byProType", c_ubyte),  # 协议类型，0-私有协议，1-松下协议，2-索尼
        ("byEnableQuickAdd", c_ubyte),  # 0 不支持快速添加  1 使用快速添加
        ("byCameraType", c_ubyte),  # 通道接入的相机类型
        ("sUserName", c_char * NAME_LEN),  # 用户名
        ("sPassword", c_char * PASSWD_LEN),  # 密码
        ("byDomain", c_char * MAX_DOMAIN_NAME),  # 设备域名
        ("struIP", NET_DVR_IPADDR),  # IP地址
        ("wDVRPort", c_uint16),  # 端口号
        ("szDeviceID", c_ubyte * DEV_ID_LEN),  # 设备ID
        ("byEnableTiming", c_ubyte),  # 0-保留，1-不启用NVR对IPC自动校时，2-启用NVR对IPC自动校时
        ("byCertificateValidation", c_ubyte),  # 证书验证
...
class NET_DVR_IPCHANINFO(Structure):
    _fields_ = [
        ("byEnable", c_ubyte),  # 该通道是否在线
        ("byIPID", c_ubyte),  # IP设备ID低8位，当设备ID为0时表示通道不可用
        ("byChannel", c_ubyte),  # 通道号
        ("byIPIDHigh", c_ubyte),  # IP设备ID的高8位
        ("byTransProtocol", c_ubyte),  # 传输协议类型 0-TCP/auto(具体有设备决定)，1-UDP 2-多播 3-仅TCP 4-auto
        ("byGetStream", c_ubyte),  # 是否对该通道取流，0-是，1-否
        ("byres", c_ubyte * 30)  # 保留
    ]
...
# NET_DVR_IPSERVER_STREAM 结构体
class NET_DVR_IPSERVER_STREAM(Structure):
    _fields_ = [
        ("byEnable", c_ubyte),  # 是否在线
        ("byRes", c_ubyte * 3),  # 保留字节
        ("struIPServer", NET_DVR_IPADDR),  # IPServer 地址
        ("wPort", c_uint16),  # IPServer 端口
        ("wDvrNameLen", c_uint16),  # DVR 名称长度
        ("byDVRName", c_ubyte * NAME_LEN),  # DVR名称
        ("wDVRSerialLen", c_uint16),  # 序列号长度
        ("byRes1", c_uint16 * 2),  # 保留字节
        ("byDVRSerialNumber", c_ubyte * SERIALNO_LEN),  # DVR序列号
        ("byUserName", c_ubyte * NAME_LEN),  # DVR 登陆用户名
        ("byPassWord", c_ubyte * PASSWD_LEN),  # DVR登陆密码
        ("byChannel", c_ubyte),  # DVR 通道
        ("byRes2", c_ubyte * 11)  # 保留字节
    ]
...
class NET_DVR_STREAM_MEDIA_SERVER_CFG(Structure):
    _fields_ = [
        ("byValid", c_ubyte),  # 是否可用
        ("byRes1", c_ubyte * 3),  # 保留字节
        ("struDevIP", NET_DVR_IPADDR),  # 流媒体服务器IP地址
        ("wDevPort", c_uint16),  # 流媒体服务器端口
        ("byTransmitType", c_ubyte),  # 传输协议类型 0-TCP，1-UDP
        ("byRes2", c_ubyte * 69)  # 保留字节
    ]
...
class NET_DVR_DEV_CHAN_INFO(Structure):
    _fields_ = [
        ...
        ("wDVRPort", c_uint16),  # 端口号
        ("byChannel", c_ubyte),  # 通道号
        ("byTransProtocol", c_ubyte),  # 传输协议类型 0-TCP，1-UDP
        ("byTransMode", c_ubyte),  # 传输码流模式 0－主码流 1－子码流
        ("byFactoryType", c_ubyte),  # 前端设备厂家类型
        ("byDeviceType", c_ubyte),  # 设备类型
        ("byDispChan", c_ubyte),  # 显示通道号
        ("bySubDispChan", c_ubyte),  # 显示通道子通道号
        ("byResolution", c_ubyte),  # 分辨率
        ("byRes", c_ubyte * 2),  # 保留字节
        ("byDomain", c_ubyte * MAX_DOMAIN_NAME),  # 设备域名
        ("sUserName", c_ubyte * NAME_LEN),  # 布防主机登陆帐号
        ("sPassword", c_ubyte * PASSWD_LEN)  # 布防主机密码
    ]


# NET_DVR_PU_STREAM_CFG 结构体
...
# NET_DVR_DDNS_STREAM_CFG 结构体
class NET_DVR_DDNS_STREAM_CFG(Structure):
    _fields_ = [
        ("byEnable", c_ubyte),  # 是否启用
        ("byRes1", c_ubyte * 3),  # 保留字节
        ("struStreamServer", NET_DVR_IPADDR),  # 流媒体服务器地址
        ("wStreamServerPort", c_uint16),  # 流媒体服务器端口
        ("byStreamServerTransmitType", c_ubyte),  # 流媒体传输协议类型 0-TCP，1-UDP
        ("byRes2", c_ubyte),
        ("struIPServer", NET_DVR_IPADDR),  # IPSERVER地址
        ("wIPServerPort", c_uint16),  # IPserver端口号
        ("byRes3", c_ubyte * 2),  # 保留字节
        ("sDVRName", c_ubyte * NAME_LEN),  # DVR名称
        ("wDVRNameLen", c_uint16),  # DVR名称长度
        ("wDVRSerialLen", c_uint16),  # 序列号长度
        ("sDVRSerialNumber", c_ubyte * SERIALNO_LEN),  # DVR序列号
        ("sUserName", c_ubyte * NAME_LEN),  # DVR登陆用户名
        ("sPassWord", c_ubyte * PASSWD_LEN),  # DVR登陆密码
        ("wDVRPort", c_uint16),  # DVR端口号
        ("byRes4", c_ubyte * 2),  # 保留字节
        ("byChannel", c_ubyte),  # DVR 通道
        ("byTransProtocol", c_ubyte),  # 传输协议类型 0-TCP，1-UDP
        ("byTransMode", c_ubyte),  # 传输码流模式 0－主码流 1－子码流
        ("byFactoryType", c_ubyte)  # 前端设备厂家类型
...
# NET_DVR_PU_STREAM_URL 结构体
class NET_DVR_PU_STREAM_URL(Structure):
    _fields_ = [
        ("byEnable", c_ubyte),  # 是否启用
        ("strURL", c_ubyte * URL_LEN),  # URL
        ("byTransPortocol", c_ubyte),  # 传输协议类型 0-tcp  1-UDP
        ("wIPID", c_uint16),  # 设备ID号
        ("byChannel", c_ubyte),  # 通道号
        ("byRes", c_ubyte * 7)  # 保留字节
    ]


# NET_DVR_HKDDNS_STREAM 结构体
class NET_DVR_HKDDNS_STREAM(Structure):
    _fields_ = [
        ("byEnable", c_ubyte),  # 是否在线
        ("byRes", c_ubyte * 3),  # 保留字节
        ("byDDNSDomain", c_ubyte * 64),  # hiDDNS服务器
        ("wPort", c_uint16),  # hiDDNS 端口
        ("wAliasLen", c_uint16),  # 别名长度
        ("byAlias", c_ubyte * NAME_LEN),  # 别名
        ("wDVRSerialLen", c_uint16),  # 序列号长度
        ("byRes1", c_ubyte * 2),  # 保留字节
        ("byDVRSerialNumber", c_ubyte * SERIALNO_LEN),  # DVR序列号
        ("byUserName", c_ubyte * NAME_LEN),  # DVR 登陆用户名
        ("byPassWord", c_ubyte * PASSWD_LEN),  # DVR登陆密码
        ("byChannel", c_ubyte),  # DVR通道
        ("byRes2", c_ubyte * 11)  # 保留字节
    ]


# NET_DVR_IPCHANINFO_V40 结构体
class NET_DVR_IPCHANINFO_V40(Structure):
    _fields_ = [
        ...
        ("byTransProtocol", c_ubyte),  # 传输协议类型 0-TCP，1-UDP，2- 多播，3-RTSP，0xff- auto
        ("byTransMode", c_ubyte),  # 传输码流模式 0－主码流 1－子码流
        ("byFactoryType", c_ubyte),  # 前端设备厂家类型
        ("byRes", c_ubyte),
        ("strURL", c_ubyte * URL_LEN)  # RTSP协议取流URL（仅RTSP协议时有效）
    ]
...
class NET_DVR_STREAM_MODE(Structure):
    _fields_ = [
        ("byGetStreamType", c_ubyte),  # 取流方式GET_STREAM_TYPE
        # 0-直接从设备取流，1-从流媒体取流
        # 2-通过IPServer获得ip地址后取流, 3.通过IPServer找到设备，再通过流媒体去设备的流
        # 4-通过流媒体由URL去取流, 5-通过hkDDNS取流
        # 6-直接从设备取流(扩展)，使用NET_DVR_IPCHANINFO_V40结构
        # 7-通过RTSP协议方式进行取流
        ("byRes", c_ubyte * 3),  # 保留字节
...
class NET_DVR_IPPARACFG_V40(Structure):
    _fields_ = [
        ...
        ("dwGroupNum", c_uint32),  # 设备支持的总组数
        ("dwAChanNum", c_uint32),  # 最大模拟通道个数
        ("dwDChanNum", c_uint32),  # 数字通道个数
        ("dwStartDChan", c_uint32),  # 起始数字通道
        ("byAnalogChanEnable", c_ubyte * MAX_CHANNUM_V30),  # 模拟通道是否启用，从低到高表示1-64通道，0表示无效 1有效
        ("struIPDevInfo", NET_DVR_IPDEVINFO_V31 * MAX_IP_DEVICE_V40),  # IP设备
        ("struStreamMode", NET_DVR_STREAM_MODE * MAX_CHANNUM_V30),  # 流模式
        ("byRes2", c_ubyte * 20)  # 保留字节
    ]
...
class NET_DVR_PLAYCOND(Structure):
    _fields_ = [
        ...
        ("byDrawFrame", c_ubyte),  # 0:不抽帧，1：抽帧
        ("byStreamType", c_ubyte),  # 码流类型，0-主码流 1-子码流 2-码流三
        ("byStreamID", c_ubyte * STREAM_ID_LEN),
        ("byCourseFile", c_ubyte),  # 课程文件0-否，1-是
        ("byDownload", c_ubyte),  # 是否下载 0-否，1-是
        ("byOptimalStreamType", c_ubyte),  # 是否按最优码流类型回放 0-否，1-是
        ("byVODFileType", c_ubyte),  # 下载录像文件，文件格式 0-PS码流格式，1-3GP格式
        ("byRes", c_ubyte * 26)  # 保留
    ]
...
# 回放码流回调函数
PLAYBACKCALLBACK = fun_ctype(None, c_long, c_ulong, POINTER(c_ubyte), c_ulong, c_void_p)
# 回放裸码流回调函数
PLAYBACKESCALLBACK = fun_ctype(None, c_long, POINTER(c_ubyte), c_void_p)

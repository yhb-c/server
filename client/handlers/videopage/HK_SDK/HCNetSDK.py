...
def get_hk_lib_path():
    """
    获取海康SDK库文件路径，兼容开发环境和PyInstaller打包后的环境
    ...
    if getattr(sys, 'frozen', False):
        ...
        return os.path.join(base_path, 'handlers', 'videopage', 'HK_SDK', 'lib')
    else:
        # 如果是开发环境运行
...
netsdkdllpath_dict = {'windows64': os.path.join(hk_lib_dir, 'HCNetSDK.dll'),
                      'windows32': os.path.join(hk_lib_dir, 'HCNetSDK.dll'),
                      'linux64': os.path.join(hk_lib_dir, 'libhcnetsdk.so'),
                      'linux32': os.path.join(hk_lib_dir, 'libhcnetsdk.so')}
...
# 设备配置命令常量
NET_DVR_CHECK_USER_STATUS = 1  # 检查用户状态
NET_DVR_GET_TIMECFG = 2  # 获取时间配置
NET_DVR_GET_DEVICECFG_V40 = 1100  # 获取设备配置V40
NET_DVR_GET_PICCFG_V40 = 1001  # 获取图像配置V40
NET_DVR_SET_PICCFG_V40 = 1002  # 设置图像配置V40
NET_DVR_GET_USERCFG_V30 = 1003  # 获取用户配置V30
NET_DVR_GET_PTZPOS = 1004  # 获取PTZ位置
NET_DVR_SET_PTZPOS = 1005  # 设置PTZ位置
NET_DVR_GET_PTZLOCKCFG = 1006  # 获取PTZ锁定配置
NET_DVR_SET_PTZLOCKCFG = 1007  # 设置PTZ锁定配置
NET_DVR_GET_GISINFO = 1008  # 获取GIS信息
NET_DVR_GET_MONTHLY_RECORD_DISTRIBUTION = 1009  # 获取月历录像分布
NET_DVR_GET_CCDPARAMCFG_EX = 1010  # 获取前端参数配置扩展
NET_DVR_SET_CCDPARAMCFG_EX = 1011  # 设置前端参数配置扩展
NET_DVR_GET_FOCUSMODECFG = 1012  # 获取聚焦模式配置
NET_DVR_SET_FOCUSMODECFG = 1013  # 设置聚焦模式配置
NET_DVR_GET_IPPARACFG_V40 = 1014  # 获取IP参数配置V40
NET_DVR_GET_PTZABSOLUTEEX = 1017  # 获取PTZ绝对位置扩展
NET_DVR_GET_AES_KEY = 1018  # 获取AES密钥
NET_DVR_GET_ALARM_RS485CFG = 1019  # 获取报警RS485配置
NET_DVR_GET_ALARMHOST_RS485_SLOT_CFG = 1020  # 获取报警主机RS485槽位配置
...
class ALARM_LCOMMAND_ENUM(Enum):
    ...
    COMM_UPLOAD_FACESNAP_mission_result = 0x1112  # 人脸抓拍结果信息,对应数据类型结构体：NET_VCA_FACESNAP_mission_result
...
# 异步登录回调函数
fLoginmission_resultCallBack = fun_ctype(None, C_LONG, C_DWORD, LPNET_DVR_DEVICEINFO_V30, c_void_p)
...
class NET_DVR_USER_LOGIN_INFO(Structure):
    _fields_ = [
        ...
        ("cbLoginmission_result", fLoginmission_resultCallBack),  # 登录状态回调函数，bUseAsynLogin 为1时有效
        ("pUser", C_LPVOID),  # 用户数据
...
class NET_DVR_SETUPALARM_PARAM(Structure):
    _fields_ = [
        ...
        # 上传报警信息类型（抓拍机支持），0-老报警信息（NET_DVR_PLATE_mission_result），1-新报警信息(NET_ITS_PLATE_mission_result)2012-9-28
        ...
        ("wTaskNo", C_WORD),
        # 任务处理号 和 (上传数据NET_DVR_VEHICLE_RECOG_mission_result中的字段dwTaskNo对应 同时 下发任务结构 NET_DVR_VEHICLE_RECOG_COND中的字段dwTaskNo对应)
...
# 人脸抓拍结果结构体。
class NET_VCA_FACESNAP_mission_result(Structure):
    _fields_ = [
        ("dwSize", C_DWORD),  # 结构体大小
...
LPNET_VCA_FACESNAP_mission_result = POINTER(NET_VCA_FACESNAP_mission_result)
...
class NET_DVR_JPEGPARA(Structure):
    _fields_ = [
        ('wPicSize', C_WORD),  # 图片尺寸：0-CIF(352*288/352*240), 1-QCIF(176*144/176*120), 2-4CIF(704*576/704*480), 3-D1(704*576/704*480), 4-UXGA(1600*1200), 5-SVGA(800*600), 6-HD720P(1280*720), 7-VGA(640*480), 8-XVGA(1280*960), 9-HD900P(1600*900), 10-HD1080P(1920*1080), 0xff-Auto(使用当前码流分辨率)
        ('wPicQuality', C_WORD),  # 图片质量：0-最好，1-较好，2-一般
    ]
...
class NET_DVR_LOCAL_TCP_PORT_BIND_CFG(Structure):
    _fields_ = [
        ('wLocalBindTcpMinPort', C_WORD),  # 本地TCP端口绑定范围最小值
        ('wLocalBindTcpMaxPort', C_WORD),  # 本地TCP端口绑定范围最大值
    ]
...
class NET_DVR_DEVICECFG_V40(Structure):
    _fields_ = [
        ('dwSize', C_DWORD),  # 结构体大小
        ('sDVRName', c_char * 64),  # 设备名称
        ('sSerialNumber', c_char * 48),  # 设备序列号
        ('byChanNum', C_BYTE),  # 模拟通道个数
        ('byStartChan', C_BYTE),  # 起始通道号
        ('byIPChanNum', C_BYTE),  # IP通道个数
        ('byZeroChanNum', C_BYTE),  # 零通道个数
        ('byMainProto', C_BYTE),  # 主码流传输协议类型：0-私有协议，1-RTSP
        ('bySubProto', C_BYTE),  # 子码流传输协议类型：0-私有协议，1-RTSP
        ('bySupport', C_BYTE),  # 能力集
        ('bySupport1', C_BYTE),  # 能力集扩展
        ('bySupport2', C_BYTE),  # 能力集扩展
        ('wDevType', C_WORD),  # 设备类型
        ('bySupport3', C_BYTE),  # 能力集扩展
        ('byMultiStreamProto', C_BYTE),  # 多码流协议类型：0-私有协议，1-RTSP
        ('byStartDChan', C_BYTE),  # 起始数字通道号
        ('byStartDTalkChan', C_BYTE),  # 起始对讲通道号
        ('byHighDChanNum', C_BYTE),  # 数字通道个数高字节
        ('bySupport4', C_BYTE),  # 能力集扩展
        ('byLanguageType', C_BYTE),  # 支持语种能力
        ('byVoiceInChanNum', C_BYTE),  # 音频输入通道数
        ('byStartVoiceInChanNo', C_BYTE),  # 音频输入起始通道号
        ('bySupport5', C_BYTE),  # 能力集扩展
        ('bySupport6', C_BYTE),  # 能力集扩展
        ('byMirrorChanNum', C_BYTE),  # 镜像通道个数
        ('wStartMirrorChanNo', C_WORD),  # 起始镜像通道号
        ('bySupport7', C_BYTE),  # 能力集扩展
        ('byRes2', C_BYTE * 238),  # 保留
    ]


# 移动侦测参数结构体（简化定义）
class NET_DVR_MOTION_STRUCT(Structure):
    _fields_ = [
        ('byEnableDisplay', C_BYTE),  # 是否启用高亮显示
        ('byRes', C_BYTE * 31),  # 保留
    ]
...
class NET_DVR_PICCFG_V40(Structure):
    _fields_ = [
        ('dwSize', C_DWORD),  # 结构体大小
        ('sChanName', c_char * 32),  # 通道名称
        ('dwShowOsd', C_DWORD),  # 预览的图像是否显示OSD
        ('struMotion', NET_DVR_MOTION_STRUCT),  # 移动侦测参数
        ('byRes', C_BYTE * 200),  # 保留
    ]


# 用户信息结构体（简化定义）
class NET_DVR_USER_INFO_STRUCT(Structure):
    _fields_ = [
        ('sUserName', c_char * 32),  # 用户名
        ('sPassword', c_char * 16),  # 密码
        ('byRes', C_BYTE * 16),  # 保留
    ]
...
class NET_DVR_USER_V30(Structure):
    _fields_ = [
        ('dwSize', C_DWORD),  # 结构体大小
        ('struUser', NET_DVR_USER_INFO_STRUCT * 32),  # 用户信息数组
        ('byRes', C_BYTE * 200),  # 保留
    ]
...
class NET_DVR_FLOW_INFO(Structure):
    _fields_ = [
        ('dwSendFlowSize', C_DWORD),  # 发送的流量数据
        ('dwRecvFlowSize', C_DWORD),  # 接收的流量数据
    ]
...
class NET_DVR_FLOW_TEST_PARAM(Structure):
    _fields_ = [
        ('dwSize', C_DWORD),  # 结构体大小
        ('lCardIndex', C_LONG),  # 卡号索引
        ('dwInterval', C_DWORD),  # 间隔时间（秒）
    ]
...
class NET_DVR_RECORD_TIME_SPAN_INQUIRY(Structure):
    _fields_ = [
        ('dwSize', C_DWORD),  # 结构体大小
        ('byType', C_BYTE),  # 查询类型：0-全部，1-定时录像，2-移动侦测录像，3-报警录像，4-手动录像
        ('byRes', C_BYTE * 3),  # 保留
    ]
...
# 码流信息结构体（用于月历录像查询）
class NET_DVR_STREAM_INFO_SIMPLE(Structure):
    _fields_ = [
...
class NET_DVR_MRD_SEARCH_PARAM(Structure):
    _fields_ = [
        ...
        ('byMonth', C_BYTE),  # 月份
        ('byRes', C_BYTE),  # 保留
        ('struStreamInfo', NET_DVR_STREAM_INFO_SIMPLE),  # 码流信息
    ]
...
class NET_DVR_MRD_SEARCH_RESULT(Structure):
    _fields_ = [
        ('dwSize', C_DWORD),  # 结构体大小
        ('byRecordDistribution', C_BYTE * 31),  # 录像分布，按位表示每天是否有录像
    ]
...
class NET_DVR_PTZPOS(Structure):
    _fields_ = [
        ('wAction', C_WORD),  # 动作：1-开始，2-停止
        ('wPanPos', C_WORD),  # 水平位置
        ('wTiltPos', C_WORD),  # 垂直位置
        ('wZoomPos', C_WORD),  # 变倍位置
    ]
...
class NET_DVR_GIS_INFO(Structure):
    _fields_ = [
        ('struPtzPos', NET_DVR_PTZPOS),  # PTZ位置信息
...
class NET_DVR_PTZ_LOCKCFG(Structure):
    _fields_ = [
        ('dwSize', C_DWORD),  # 结构体大小
        ('byWorkMode', C_BYTE),  # 工作模式：0-解锁，1-锁定
        ('byRes', C_BYTE * 3),  # 保留
    ]
...
class NET_DVR_CORRIDOR_MODE(Structure):
    _fields_ = [
        ('byEnableCorridorMode', C_BYTE),  # 是否开启旋转
        ('byRes', C_BYTE * 31),  # 保留
    ]
...
class NET_DVR_CAMERAPARAMCFG_EX(Structure):
    _fields_ = [
        ('dwSize', C_DWORD),  # 结构体大小
        ('struCorridorMode', NET_DVR_CORRIDOR_MODE),  # 走廊模式
        ('byRes', C_BYTE * 200),  # 保留
    ]
...
class NET_DVR_FOCUSMODE_CFG(Structure):
    _fields_ = [
        ('dwSize', C_DWORD),  # 结构体大小
        ('byFocusMode', C_BYTE),  # 聚焦模式
        ('byFocusDefinitionDisplay', C_BYTE),  # 聚焦清晰度显示
        ('byFocusSpeedLevel', C_BYTE),  # 聚焦速度等级
        ('byRes', C_BYTE),  # 保留
    ]
...
class NET_DVR_CHAN_INFO(Structure):
    _fields_ = [
        ('byIPID', C_BYTE),  # IP ID
        ('byIPIDHigh', C_BYTE),  # IP ID 高字节
        ('byEnable', C_BYTE),  # 是否启用
        ('byRes', C_BYTE * 61),  # 保留
    ]

# 码流模式联合体（简化定义）
class NET_DVR_STREAM_MODE_UNION(Structure):
    _fields_ = [
        ('struChanInfo', NET_DVR_CHAN_INFO),  # 通道信息
        ('byRes', C_BYTE * 64),  # 保留
    ]
...
class NET_DVR_STREAM_MODE(Structure):
    _fields_ = [
        ('byGetStreamType', C_BYTE),  # 获取码流类型
        ('uGetStream', NET_DVR_STREAM_MODE_UNION),  # 联合体
        ('byRes', C_BYTE * 64),  # 保留
    ]

# IP设备信息结构体
class NET_DVR_IP_DEV_INFO(Structure):
    _fields_ = [
...
class NET_DVR_IPPARACFG_V40(Structure):
    _fields_ = [
        ('dwSize', C_DWORD),  # 结构体大小
        ('dwStartDChan', C_DWORD),  # 起始数字通道号
        ('dwDChanNum', C_DWORD),  # 数字通道个数
        ('struStreamMode', NET_DVR_STREAM_MODE * 32),  # 码流模式数组
        ('struIPDevInfo', NET_DVR_IP_DEV_INFO * 32),  # IP设备信息数组
        ('byRes', C_BYTE * 200),  # 保留
    ]
...
class NET_DVR_CHECK_DEV_STATE(Structure):
    _fields_ = [
        ('dwTimeout', C_DWORD),  # 超时时间（毫秒）
        ('fnStateCB', c_void_p),  # 状态回调函数
    ]
...
class NET_DVR_PTZABSOLUTEEX_CFG(Structure):
    _fields_ = [
        ('dwFocalLen', C_DWORD),  # 焦距范围
        ('struPTZCtrl', NET_DVR_PTZ_CTRL),  # PTZ控制
    ]
...
class NET_DVR_GBT28181_CHANINFO_CFG(Structure):
    _fields_ = [
        ('dwSize', C_DWORD),  # 结构体大小
        ('szVideoChannelNumID', c_char * 64),  # 视频通道编号ID
    ]
...
# 通道状态结构体
class NET_DVR_CHAN_STATIC(Structure):
    _fields_ = [
        ('byRecordStatic', C_BYTE),  # 是否在录像
        ('byRes', C_BYTE * 63),  # 保留
    ]

# 设备工作状态结构体（用于回调）
class NET_DVR_DEV_WORK_STATE(Structure):
    _fields_ = [
        ('dwDeviceStatic', C_DWORD),  # 设备状态
        ('struChanStatic', NET_DVR_CHAN_STATIC * 32),  # 通道状态数组
    ]
...
# 设备工作状态回调函数类型
DEV_WORK_STATE_CB = fun_ctype(c_bool, c_void_p, C_LONG, POINTER(NET_DVR_DEV_WORK_STATE))

...
'''
    ...
    # 保存测试脚本
    script_path = "test/server_rtsp_test.py"
    with open(script_path, 'w', encoding='utf-8') as f:
        f.write(test_script)
    
    print(f"RTSP测试脚本已创建: {script_path}")
    return script_path

def upload_and_run_test():
    """上传并运行测试脚本"""
    print("\n=== 上传并运行RTSP测试 ===")
    ...
    # 创建测试脚本
    script_path = create_rtsp_test_script()
    
    # 上传测试脚本
    remote_script = "/home/lqj/liquid/server_rtsp_test.py"
    ...
    ssh_commands = [
        ...
        f"cd /home/lqj/liquid && source ~/anaconda3/bin/activate liquid && export LD_LIBRARY_PATH=/home/lqj/liquid/sdk/hikvision/lib:$LD_LIBRARY_PATH && python {remote_script}"
    ]
...
def check_test_results():
    ...
    # 检查结果文件
    check_cmd = "cat /home/lqj/liquid/rtsp_test_result.json 2>/dev/null || echo '结果文件不存在'"
...
def main():
    """主函数"""
    print("开始测试服务端RTSP流捕获功能")
    
    try:
        # 1. 上传并运行测试
        if upload_and_run_test():
            # 2. 等待测试完成
            print("\n等待测试完成...")
            time.sleep(5)

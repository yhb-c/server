...
"""
液位线绘制脚本
读取测试集图片，使用液位检测引擎检测液位线，并将液位线绘制在图片上保存
"""
...
def draw_liquid_line(image, detection_result, annotation_config):
    """在图片上绘制液位线和检测框（适配裁剪后的ROI图片）"""
    img_draw = image.copy()
...
def main():
    ...
    for img_path in image_files:
        ...
        # 绘制液位线
        img_with_line = draw_liquid_line(image, detection_result, annotation_config)

        # 保存结果
        output_path = output_dir / img_path.name
        cv2.imwrite(str(output_path), img_with_line)
